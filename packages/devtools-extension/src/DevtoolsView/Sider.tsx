...
import {
  currentView, InstanceDetails,
  instanceDetails,
  InstancePlaceholder,
  InstancesList,
  instancesList,
} from "./sources";
import {Status, useSource, useAsyncState, State, mapFlags} from "react-async-states";
      ...
      <div className="sider-root">
        {
          Object.entries(instancesToDisplay)
            .map(([key, instances]) => <section key={key}>
      ...
      <section>
        ...
        <div className="sider-group">
          {entries.map(([uniqueId, instance]) => (
            <InstanceDetailsView current={uniqueId == data} key={uniqueId}
                                 instance={instance}/>
          ))}
        </div>
      </section>
...
const InstanceDetailsView = React.memo(function InstanceDetailsView(props: { current: boolean, instance: InstancePlaceholder }) {
  ...
  let subscriptionsFlags = state.data?.subscriptions?.map(t => mapFlags(t.flags).join(",")).join('|');
  ...
  return (
    ...
    >
      ...
      <span
        ...
        className="sider-key-subscriptions">{Object.is(subscriptionsCount, Number.NaN) ? '' : subscriptionsCount}</span>
    </button>
  );
});
