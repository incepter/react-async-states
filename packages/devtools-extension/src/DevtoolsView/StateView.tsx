...
import { Status, useAsync } from "react-async-states";
...
function StateDetails({ id }) {
	let [displayedTabs, setDisplayedTabs] =
		React.useState<Record<string, boolean>>(emptyObject);
	let { state } = useAsync({ source: instanceDetails, lane: id }, [id]);
...
const JournalDisplay = function JournalDisplay({
	...
	let filteredValues = Object.values(filteredData).sort(
		sortByEventIdDesc
	) as any[];
	...
	function handleSelectChange(e: React.ChangeEvent<HTMLSelectElement>) {
		...
		if (
			!selectedTypes.find((option: DevtoolsJournalEvent) => option === value)
		) {
			return setSelectedTypes([...selectedTypes, value]);
		} else {
			return setSelectedTypes(
				selectedTypes.filter((option: DevtoolsJournalEvent) => option !== value)
			);
		}
	}
		...
		<div className="journal-root">
			<div className="scroll-y-auto">
				<div className="journal-filter">
					<span>
						Available: ({allLogs.length}), shown: ({filteredValues.length})
					</span>
					<div style={{ display: "flex", gap: 4 }}>
						<button
							className="devtools-button as-default-button"
						...
						<button
							className="devtools-button as-default-button"
							onClick={() =>
								setSelectedTypes(Object.values(DevtoolsJournalEvent))
							}
						>
							Select all
						</button>
					...
					<ul style={{ marginTop: 8, paddingBottom: "1rem" }}>
						{filteredValues.map((entry) => (
							<li style={{ width: "100%" }} key={entry.eventId}>
								<button
									style={{
										width: "100%",
										borderRadius: 100,
			...
			<div className="journal-content scroll-y-auto">
				{currentJournal && (
					<Json level={1} name={`journal`} src={filteredData[currentJournal]} />
				)}
			</div>
		</div>
	);
...
function ChangeState({ uniqueId, displayKey }) {
	...
	return (
		<>
			<button className="devtools-button" onClick={() => setOpen(true)}>
				Change state
			</button>
			{open && (
				<div
