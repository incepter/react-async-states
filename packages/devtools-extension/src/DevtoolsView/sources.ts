...
export let devtoolsInfo = createSource<Info, any, any>(
	"info",
	devtoolsInfoProducer,
	{
...
export let instanceDetails = createSource<InstanceDetails | null, any, any>(
	"instance-details",
	instanceDetailsProducer,
	{
		initialValue: null,
		hideFromDevtools: true,
	}
...
function instanceDetailsProducer(
	props: ProducerProps<InstanceDetails | null, any[], any>
) {
	let { data: port } = gatewaySource.getState();
	if (!port) {
		throw new Error("Couldn't know port, this is a bug");
...
export type InstancePlaceholder = {
	uniqueId: number;
	key: string;
	status?: Status;
	lastUpdate?: number;
...
export let instancesList = createSource<InstancesList, any, any>(
	"instances",
	null,
	{
		initialValue: {},
...
export let gatewaySource = createSource("gateway", gatewayProducer, {
	hideFromDevtools: true,
});
	...
	port.onMessage.addListener((message) => {
		...
		switch (message.type) {
			case DevtoolsEvent.setKeys: {
				...
				let newKeys = Object.entries(payload as Record<number, string>).reduce(
					(acc, [uniqueId, key]) => {
...
function applyPartialUpdate(message) {
	...
	switch (eventType) {
		case DevtoolsJournalEvent.run: {
			let currentDetails = (currentSource.getState().data ||
				{}) as InstanceDetails;
			currentDetails.journal =
				((currentDetails.journal || []).push(message.payload),
				currentDetails.journal);
		...
		case DevtoolsJournalEvent.update: {
			...
			prevPlaceholder.lastUpdate =
				message.payload.eventPayload.newState.timestamp;
		...
		case DevtoolsJournalEvent.subscription: {
			let currentDetails = (currentSource.getState().data ||
				{}) as InstanceDetails;
			currentDetails.subscriptions =
				((currentDetails.subscriptions || []).push(
		...
		case DevtoolsJournalEvent.unsubscription: {
			...
			currentDetails.subscriptions = (
				currentDetails.subscriptions || []
			).filter((t) => t.key !== message.payload.eventPayload);
			currentDetails.journal =
				((currentDetails.journal || []).push(message.payload),
				currentDetails.journal);
