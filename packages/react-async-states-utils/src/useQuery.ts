...
import {MixedConfig, State, Status} from "async-states";
import {useAsyncState} from "react-async-states";
import {UseAsyncState} from "async-states";
...
type MappedConfig<T, E, R, S = State<T, E, R>> = {
    producerConfig: ProducerConfig<T, E, R>,
    config: MixedConfig<T, E, R, S>,
    deps: any[]
}

function buildUseAsyncStateConfig<T, E, R, K extends QueryKey>(options: Options<T, E, K>) : MappedConfig<T, E, R, State<T, E, R>>{
...
function refreshProducerAndConfig<T, E, R, S, K extends QueryKey>(
  ...
  options: Options<T, E, K>
    ...
    source.patchConfig(mappedConfig.producerConfig);

}

function mapResult<T, E, R, K extends QueryKey>(
  result: UseAsyncState<T, E, R>,
  options: Options<T, E, K>
): Result<T, E> {
...
export function useQuery<TQueryFnData, E, T, TQueryData, K extends QueryKey, R>(
  options: Options<T, E, K>
): Result<T, E> {
    ...
    refreshProducerAndConfig(result.source!, mappedConfig, options);
