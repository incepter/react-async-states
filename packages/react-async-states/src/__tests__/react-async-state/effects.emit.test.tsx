...
describe("should emit from producer", () => {
	...
	it("should stop emitting after abort", async () => {
		...
		function Component() {
			...
			return (
				<>
					<button data-testid="abort" onClick={() => abort("tt")}>
						abort
					</button>
	...
	it("should emit before resolve and do nothing", async () => {
		...
		expect(mockedErrorLog).toHaveBeenCalledWith(
			"Called props.emit before the producer resolves. This is not supported in the library and will have no effect"
		);
