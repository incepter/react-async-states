...
export function useSelector<T>(
	...
	selector?: SimpleSelector<unknown, unknown, unknown[], T>
): T;
export function useSelector<T>(
	keys: BaseSelectorKey[],
...
export function useSelector<T>(
	...
	selector:
		| SimpleSelector<unknown, unknown, unknown[], T>
		| ArraySelector<T>
		| FunctionSelector<T> = identity
): T {
	let caller;
	if (__DEV__) {
	...
	let keysArray = React.useMemo(
		() => readKeys(keys, pool),
		[guard, keys, pool]
	);
	let instances = React.useMemo(
		() => resolveInstances(keysArray, pool),
		[keysArray]
	);

	let [state, setState] = React.useState<{ value: T; guard: number }>(
		computeState
	);
...
function readKeys(
	...
	pool: PoolInterface
): (string | Source<unknown, unknown, unknown[]>)[] {
	if (isFunction(keys)) {
		const availableKeys = pool.instances.keys();
		return readKeys(
			(keys as UseSelectorFunctionKeys)(Array.from(availableKeys)),
			pool
		);
	}
...
function resolveInstances(
	keysArray: (string | Source<unknown, unknown, unknown[]>)[],
	pool: PoolInterface
): Record<string, StateInterface<unknown, unknown, unknown[]> | undefined> {
	return keysArray.reduce((result, current) => {
		...
		return result;
	}, {} as Record<string, StateInterface<unknown, unknown, unknown[]> | undefined>);
}
...
function selectValue<T>(
	...
	instances: Record<
		string,
		StateInterface<unknown, unknown, unknown[]> | undefined
	>
): T {
	if (isFunction(keys)) {
		const selectorParam = Object.entries(instances).reduce(
			(result, [key, maybeInstance]) => {
				if (!maybeInstance) {
					result[key] = { key };
				} else {
	...
	return (selector as ArraySelector<T>).apply(
		null,
		Object.entries(instances).map(([key, maybeInstance]) => {
			if (!maybeInstance) {
				return { key };
			} else {
...
function subscribeAndWatch<T>(
	...
	unwatch = pool.listen(function (value: InstanceOrNull<any>, key: string) {
		if (
			resolvedInstances.hasOwnProperty(key) &&
	...
	let unsubscribe = resolvedEntries.map(([, maybeInstance]) =>
		maybeInstance?.actions.subscribe({
			cb: onUpdate,
			flags: undefined,
			key: subscriptionKey,
		})
