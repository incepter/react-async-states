...
function fetchUser(
	props: ProducerProps<AxiosResponse<User>, Error, never, [string]>
) {
	if (!props.args[0]) {
		props.abort!();
...
export default function Welcome() {
	let result = app.users.findById
		.inject(fetchUser, userDetailsConfig)
		.useAsyncState();
	let { source, state, run } = result;

	let searchedUserId: string | null =
		state.status === Status.initial ? null : state.props.args![0];

	let summary =
		state.status === Status.pending
			? `Fetching user ${searchedUserId}`
			: state.status === Status.success
			? `User ${state.data.data.username} details`
			: state.status === Status.error
			? `User ${searchedUserId} fetch error`
			: "Search results";
					...
					{JSON.stringify(
						{
							status: state.status,
							data:
								state.status === Status.error ? state.data.message : state.data,
						},
						null,
						4
					)}
