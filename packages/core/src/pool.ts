...
function createPool(name: string, context: LibraryPoolsContext): PoolInterface {
	...
	function set<T, E, A extends unknown[]>(
		key: string,
	...
	function listen<T, E, A extends unknown[]>(
		notify: WatchCallback<T, E, A>
	): AbortFn {
		let didClean = false;
		let index = ++meter;

		function cb(argv: StateInterface<T, E, A> | null, notifKey: string) {
			if (!didClean) {
	...
	function watch<T, E, A extends unknown[]>(
		key: string,
		notify: WatchCallback<T, E, A>
	): AbortFn {
		if (!watchers[key]) {
	...
	function notifyWatchers<T, E, A extends unknown[]>(
		key: string,
...
export function requestContext(context: unknown): LibraryPoolsContext {
	...
	if (!executionContext && isServer) {
      ...
      `);
		...
		throw new Error(
			"You should always provide an execution context in the server"
		);
	}
	if (!executionContext) {
		if (__DEV__) {
