import {AsyncState, createSource} from "../../AsyncState";
...
describe("Create instances in different pools", () => {
	...
	it("should reuse the same instance in the same pool", () => {
		consoleErrorSpy.mockClear();
		let source1 = createSource("key2", null, { pool: "pool-1" });
		let source2 = createSource("key2", null, { pool: "pool-1" });
		expect(source1).toBe(source2);
	});

	it("should use a different instance from different pools", () => {
		consoleErrorSpy.mockClear();
		let source1 = createSource("key3", null, { pool: "pool-1" });
		let source2 = createSource("key3", null, { pool: "pool-2" });
	...
	it("should merge payload inside all pools instances", () => {
		...
		let src = createSource("test-key", null, { pool: "test-pool" });
		let src2 = createSource("test-key2", null, { pool: "test-pool" });
